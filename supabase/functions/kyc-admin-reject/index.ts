// supabase/functions/kyc-admin-reject/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const { error: updErr } = await supabase
      ...
      .update({ status: 'rejected', reviewed_at: new Date().toISOString(), reviewer_id: null, rejection_reason: reason })
      .eq('id', submission_id);
    if (updErr) {
    ...
    const { error: histErr } = await supabase
      ...
      .insert({ submission_id, action: 'rejected', actor_id: null, reason });
    if (histErr) {
    ...
    if (!subErr && sub?.user_id) {
      await notifyUser(sub.user_id as string, 'KYC refusé', `Votre KYC a été refusé: ${reason}`, { type: 'kyc_rejected' });
    }
